...
const ModalCreateManagerSport = ({ isOpen, closeModal, sportId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await TokenRequest.put(
        `/sports/v18/update/${sportId}/managers`,
  ...
  const handleRemoveManager = async (managerId) => {
    try {
      const response = await TokenRequest.put(
        `/sports/v18/remove/${sportId}/manager`,
        { userId: managerId }
      );
      if (response.status === 200) {
  ...
  useEffect(() => {
    const fetchSportData = async () => {
      try {
        ...
        const unsubscribe = onSnapshot(sportRef, async (sportSnap) => {
          if (sportSnap.exists()) {
            const sportData = sportSnap.data();
            const createByRef = await getDoc(
              doc(db, "users", sportData.UserId)
            );
            const managers = sportData.managers;
            const managersDataPromises = managers.map(async (managerId) => {
              if (managerId !== createByRef.id) {
                const managerSnap = await getDoc(doc(db, "users", managerId));
                if (managerSnap.exists()) {
                  const managerData = managerSnap.data();
                  delete managerData.password;
                  delete managerData.expo_push_token;
                  // Fetch role information for each manager

                  const roleSnap = await getDoc(
                    doc(db, "roles", managerData.RoleId)
                  );
            ...
            console.log("Sport document does not exist");
          }
        });

        return () => unsubscribe();
      } catch (error) {
        console.error("Error fetching sport document:", error);
      }
    };
  ...
  return (
    ...
    >
      <div className="modal-header flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Add Manager to Sport</h2>
        <button
          onClick={closeModal}
...
export default ModalCreateManagerSport;
