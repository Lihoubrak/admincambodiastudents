...
const ModalCreateManagerUniversity = ({ isOpen, closeModal, universityId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await TokenRequest.put(
        `/schools/v4/update/${universityId}/managers`,
  ...
  const handleRemoveManager = async (managerId, role) => {
    try {
      const response = await TokenRequest.put(
        `/schools/v4/remove/${universityId}/manager`,
        { userId: managerId, role } // Pass both userId and role obtained from the manager object
      );
      if (response.status === 200) {
  ...
  useEffect(() => {
    const fetchSchoolData = async () => {
      try {
        const schoolRef = doc(db, "schools", universityId);
        const unsubscribe = onSnapshot(schoolRef, async (schoolSnap) => {
          if (schoolSnap.exists()) {
            ...
            const managersDataPromises = managers.map(async (manager) => {
              ...
              const managerSnap = await getDoc(doc(db, "users", userId));
  ...
  return (
    ...
    >
      <div className="modal-header flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Add Manager to University</h2>
        <button
          onClick={closeModal}
...
export default ModalCreateManagerUniversity;
