...
const ModalCreateManagerProgram = ({ isOpen, closeModal, eventId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await TokenRequest.put(
        `/events/v9/update/${eventId}/managers`,
  ...
  const handleRemoveManager = async (managerId) => {
    try {
      const response = await TokenRequest.put(
        `/events/v9/remove/${eventId}/manager`,
        { userId: managerId }
      );
      if (response.status === 200) {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        const unsubscribe = onSnapshot(eventRef, async (eventSnap) => {
          if (eventSnap.exists()) {
            const eventData = eventSnap.data();
            const { UserId, managers } = eventData;
            // Fetch managers data
            const managersDataPromises = managers.map(async (managerId) => {
              if (managerId !== UserId) {
                ...
                if (managerSnap.exists()) {
                  ...
                  const roleId = managerData.RoleId;
                  ...
                  console.log(`User with ID ${managerId} does not exist`);
                  return null;
                }
              }
            });
  ...
  return (
    ...
    >
      <div className="modal-header flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Add Manager to Program</h2>
        <button
          onClick={closeModal}
...
export default ModalCreateManagerProgram;
